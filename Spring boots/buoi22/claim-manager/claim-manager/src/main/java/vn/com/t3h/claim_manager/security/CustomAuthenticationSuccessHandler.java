...
////    Tao ra cookie, them token vaof cookie
...
public class CustomAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
    ...
    // Tạo cookie, thêm token vào cookie
    Cookie jwtCookie = new Cookie(JWT.JWT.name(), jwt);
    jwtCookie.setPath("/");
    jwtCookie.setHttpOnly(true);
    jwtCookie.setSecure(false); // Chỉ trong môi trường phát triển, đặt thành true nếu sử dụng HTTPS
    jwtCookie.setMaxAge(24 * 60 * 60); // Thời gian sống của cookie
    response.addCookie(jwtCookie);

    // Lấy ra URL để điều hướng theo role
    String urlRedirect = loginService.processAfterLoginSuccess();
    response.sendRedirect(urlRedirect);
  }
}
